...
wav_file_name = 'wav\\bfsk_out_smooth.wav'
...
# Размер потока данных с преамбулой и словом синхронизации, бит
...
# Form a smooth rise and fall of the frequency
# Формируем нарастание и спад частоты
phase_freq_difference = ((bit_1_frequency - bit_0_frequency) / 2) / bit_samplerate_period_per_bit
bit_0_sin_rise_samples = np.sin(2 * np.pi * ((bit_0_frequency + phase_freq_difference * bit_sin_samples_index) / transmit_bitrate) * bit_sin_samples_index / bit_samplerate_period_per_bit)
bit_1_sin_fall_samples = np.sin(2 * np.pi * ((bit_1_frequency - phase_freq_difference * bit_sin_samples_index) / transmit_bitrate) * bit_sin_samples_index / bit_samplerate_period_per_bit)

# plt.figure("Во времени")
# plt.plot(bit_sin_samples_index, bit_0_sin_samples + 1, 'g', bit_0_sin_rise_samples + 1, 'b', bit_1_sin_samples - 1, 'g', bit_1_sin_fall_samples - 1, 'r')
# plt.grid()
# plt.show()
...
for i in range(int(output_signal_sample_count + bit_samplerate_period_per_bit)):
    ...
    if i >= bit_samplerate_period_per_bit:
        ...
        # And from the frequency change flags
        # Частота зависит от значения бита
        # И от флагов изменения частот
        if bit_value == 0:
            if bit_value_rise_flag == 0:
                output_signal[output_signal_sample_counter] = bit_0_sin_samples[phase_cnt]
            else:
                output_signal[output_signal_sample_counter] = bit_0_sin_rise_samples[phase_cnt]
        else:
            if bit_value_fall_flag == 0:
                output_signal[output_signal_sample_counter] = bit_1_sin_samples[phase_cnt]
            else:
                output_signal[output_signal_sample_counter] = bit_1_sin_fall_samples[phase_cnt]

        # Set the fall flag near the end of the data bit
        # Выставим флаг спада в близи конца данных
        if output_signal_sample_counter >= (output_signal_sample_count - bit_samplerate_period_per_bit):
            bit_value_fall_flag = 1

        # Checking the bit value change
        # Смотрим смену текущего значения сигнала
        if (bit_value != bit_value_new) and (bit_value_rise_flag == 0) and (bit_value_fall_flag == 0):
            bit_value_rise_flag = 1
            bit_value_fall_flag = 1
